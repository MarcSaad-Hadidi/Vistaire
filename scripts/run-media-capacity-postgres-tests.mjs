...
if (process.env.VISTAIRE_MEDIA_CAPACITY_POSTGRES_TEST !== "1" && process.env.CI !== "true") {
  throw new Error(
    "Refusing PostgreSQL capacity tests outside CI: set VISTAIRE_MEDIA_CAPACITY_POSTGRES_TEST=1 for a dedicated ephemeral test database."
  );
}
...
const version = spawnSync(
  "psql",
  [...args, "--tuples-only", "--no-align", "--command", "select current_setting('server_version_num');"],
...
if (serverVersion < 170000 || serverVersion >= 180000) {
  throw new Error(`PostgreSQL 17 is required; server_version_num=${serverVersion || "unknown"}.`);
}
const result = spawnSync(
  "psql",
  [...args, "--file", path.join(root, "tests/postgres/media-capacity/run.sql")],
...
console.log(result.stdout.trim() || "Media capacity PostgreSQL concurrency and security checks passed.");
