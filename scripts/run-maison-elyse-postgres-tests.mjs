...
if (process.env.VISTAIRE_MAISON_ELYSE_POSTGRES_TEST !== "1" && process.env.CI !== "true") {
  throw new Error(
    "Refusing to initialize PostgreSQL outside CI: set VISTAIRE_MAISON_ELYSE_POSTGRES_TEST=1 for a dedicated ephemeral test database."
  );
}
