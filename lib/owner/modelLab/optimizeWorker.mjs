...
async function optimize() {
  ...
  if (preset.textureMax !== null) {
    transforms.push(
      textureCompress({
        encoder: sharp,
        targetFormat: preset.textureFormat ?? undefined,
        resize: [preset.textureMax, preset.textureMax],
        resizeFilter: TextureResizeFilter.LANCZOS3,
        quality: preset.textureQuality ?? 92,
        effort: preset.textureEffort ?? undefined,
        limitInputPixels: true
      })
    );
  }

  if (preset.geometryCompression === "reorder") {
    transforms.push(reorder({ encoder: MeshoptEncoder }));
  } else if (preset.geometryCompression === "meshopt") {
    transforms.push(
      reorder({ encoder: MeshoptEncoder }),
      meshopt({
        encoder: MeshoptEncoder,
        level: preset.meshoptLevel ?? "medium"
      })
    );
  }
