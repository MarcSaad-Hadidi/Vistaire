...
function rowsFromEvents(events) {
  ...
  return {
    ...
    searchRows: sortSearchRowsByCount(
      [...searches].map(([search_query, search_count]) => ({ search_query, search_count }))
    ),
...
export function sortSearchRowsByCount(rows) {
  return [...rows].sort((left, right) => {
    const countDelta =
      numberFrom(right, ["search_count", "count"]) -
      numberFrom(left, ["search_count", "count"]);
    if (countDelta !== 0) return countDelta;
    return stringFrom(left, ["search_query", "term"]).localeCompare(
      stringFrom(right, ["search_query", "term"]),
      "en"
    );
  });
}
