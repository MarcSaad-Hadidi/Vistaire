...
export function buildAdminAnalyticsState(input:AdminAnalyticsInput):AdminAnalyticsState{
 ...
 return{kind:"real",completeness:relevant<20?"limited-sample":"complete",observationWindow:input.observationWindow,lastUpdatedAt:last,freshness:age<=3600000?"fresh":age<=86400000?"delayed":"stale",coverage:{menuOpened:menuCount>0||proven,dishOpened:dishCount>0||proven},metrics:(input.metrics??[{id:"menu-opens",value:menuCount},{id:"dish-opens",value:dishCount}]).map(addComparisonEvidence),activitySeries:buckets.length>=2?buckets:[],categoryBreakdown:categories,topDishes:rankedDishes,searches:searchCounts,immersive,funnel:qualifying.length>=20?{kind:"measured",sessions:qualifying.length,dishOpened:converted,rate:converted/qualifying.length}:{kind:"unsupported"},comparison:null};
}
