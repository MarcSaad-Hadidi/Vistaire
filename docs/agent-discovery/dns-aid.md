...
- DNS-AID namespace: `_agents.www.vistaire.ca`
...
`_index._agents` points agents to the HTTPS endpoint that serves `/.well-known/api-catalog`, `/.well-known/agent-skills/index.json`, `/auth.md`, and `/.well-known/mcp/server-card.json`.
...
- Publish the `_index._agents.www.vistaire.ca` records above in the `vistaire.ca` zone.
