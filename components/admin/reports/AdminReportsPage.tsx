...
export function AdminReportsPage({ report }: { report: AdminReportModel }) {
  ...
  return <div className={styles.report}>
    <header className={styles.reportHeader}>
      <p className={styles.eyebrow}>{fr ? "Périmètre de preuve" : "Evidence scope"}</p>
      <div className={styles.period}><span>{fr ? "Période alignée" : "Aligned period"}</span><strong>{periodLabel(report)}</strong><small>{report.window.timezone}</small></div>
    </header>
