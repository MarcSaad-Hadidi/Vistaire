...
export function ReportAvailabilityChanges({ report }: { report: AdminReportModel }) {
  if (!report.availabilityChanges.value) return <div className={styles.emptyState} role="status"><strong>{report.locale === "fr" ? "Ã‰volution des disponibilitÃ©s" : "Availability changes"}</strong><p>{report.availabilityChanges.copy}</p></div>;
