...
export function ReportSearches({ report }: { report: AdminReportModel }) {
  if (!report.searches.value) return <div className={styles.emptyState} role="status"><strong>{report.locale === "fr" ? "Recherches prot̩g̩es" : "Privacy-protected searches"}</strong><p>{report.searches.copy}</p></div>;
