...
export function ReportFilters({ report }: { report: AdminReportModel }) {
  ...
  const rangeLabel = { today: fr ? "Aujourd’hui" : "Today", "7d": fr ? "7 jours" : "7 days", "30d": fr ? "30 jours" : "30 days" };
  const serviceLabel = { all: fr ? "Tous les services" : "All services", lunch: fr ? "Déjeuner" : "Lunch", dinner: fr ? "Dîner" : "Dinner" };
  return (
    <nav className={styles.filters} aria-label={fr ? "Filtres du rapport" : "Report filters"} data-report-print-hidden>
      <div className={styles.filterGroup} aria-label={fr ? "Période" : "Period"}>
