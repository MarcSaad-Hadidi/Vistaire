...
export type InteractionAction =
  ...
  | { type: "leave" | "outside" | "blur" }
  | { type: "key"; key: string; count: number; columns?: number };
...
export function interactionReducer(state: InteractionState, action: InteractionAction): InteractionState {
  ...
  if (action.type === "blur") return { active: null, pinned: false };
  ...
  if (action.key === "Tab") return { active: null, pinned: false };
  ...
  const columns = Math.max(1, action.columns ?? 1);
  if (action.key === "ArrowDown" && columns > 1) return { active: Math.min(action.count - 1, Math.max(0, current) + columns), pinned: state.pinned };
  if (action.key === "ArrowUp" && columns > 1) return { active: Math.max(0, Math.max(0, current) - columns), pinned: state.pinned };
