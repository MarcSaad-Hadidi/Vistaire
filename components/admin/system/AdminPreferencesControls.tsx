...
export function AdminPreferencesControls({ preferences }: { preferences: AdminPreferences }) {
  ...
  return (
    <details key={`${preferences.locale}-${preferences.theme}`} className={styles.adminPreferences}>
      <summary aria-label={preferences.locale === "fr" ? "Préférences d’affichage" : "Display preferences"}>
        <span>{preferences.locale.toUpperCase()}</span>
        <span aria-hidden="true">·</span>
        <span>{preferences.theme === "light" ? labels.light : labels.dark}</span>
      </summary>
