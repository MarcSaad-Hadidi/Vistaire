...
export function InsightsSearchRows({ rows }: { rows: SearchRow[] }) {
  ...
  return <ol className={styles.searchRows}>{rows.map((row) => <li key={row.term} data-insights-search-row>
    <button type="button" onClick={() => setActive(active === row.term ? null : row.term)} onBlur={() => setActive(null)} aria-describedby={`search-tip-${row.term}`}><span>{row.term}</span><Sparkline values={row.daily} label={`Tendance de ${row.term}`}/><strong>{row.count}</strong><small>{change(row.changeRate)}</small></button>
