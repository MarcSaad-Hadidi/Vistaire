...
export function AdminInsightsPage({ data, range }: { data: AdminDashboardData; range: AdminDashboardRange }) {
  ...
  return <AdminShell restaurantName={data.restaurant.name} menuPath={data.restaurant.publicMenuPath} active="insights">
    <header className={styles.insightsHeader}><Link className={styles.srBack} href="/admin">Retour au tableau de bord</Link><nav aria-label="Période analysée">{(["today-utc","7d","30d"] as AdminDashboardRange[]).map((item) => <Link key={item} href={`/admin/insights?range=${item}`} aria-current={range === item ? "page" : undefined}>{item === "today-utc" ? "24 h" : item}</Link>)}</nav></header>
