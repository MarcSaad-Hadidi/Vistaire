...
export function AdminOverview({ data, range }: { data: AdminDashboardData; range: AdminDashboardRange }) {
  ...
  return <AdminShell restaurantName={data.restaurant.name} active="overview" actions={<><Link className={styles.headerAction} href={data.restaurant.publicMenuPath} prefetch={false}><ExternalIcon/>Ouvrir le menu</Link><Link className={styles.headerAction} href="/admin/insights"><InsightsIcon/>Voir les analyses</Link></>}>
    ...
    <div className={styles.overviewGrid}><AdminPanel className={styles.activity} title="Activité du menu" eyebrow={rangeLabel(range)} action={<Link href="/admin/insights">Détails</Link>}><AdminActivityChart evidence={panels?.currentDaily ?? unavailable}/></AdminPanel><AdminPanel className={styles.top} title="Plats les plus consultés" eyebrow="Classement"><AdminTopDishes evidence={panels?.ranking ?? unavailable} names={names}/></AdminPanel><AdminPanel className={styles.moment} title="Moments forts" eyebrow="Service · UTC">{panels?.serviceWindows?.kind === "supported" ? <p className={styles.bigStat}>{[...panels.serviceWindows.data.windows].sort((a,b) => b.count-a.count)[0]?.label}<strong>{[...panels.serviceWindows.data.windows].sort((a,b) => b.count-a.count)[0]?.count} événements</strong></p> : <AdminEvidenceState kind={(panels?.serviceWindows ?? unavailable).kind as "insufficient" | "unavailable"} reason={(panels?.serviceWindows ?? unavailable).reason}/>}</AdminPanel><AdminPanel className={styles.category} title="Par catégorie" eyebrow="Consultations">{categories.kind === "supported" ? <ul className={styles.categoryBars}>{categories.data.slice(0,3).map((item) => <li key={item.slug}><span>{item.slug}</span><i style={{ "--value": `${totalCategories ? item.count / totalCategories * 100 : 0}%` } as React.CSSProperties}/><strong>{item.count}</strong></li>)}</ul> : <AdminEvidenceState kind={categories.kind} reason={categories.reason}/>}</AdminPanel><AdminPanel className={styles.availability} title="Disponibilité des plats" eyebrow="Carte en direct"><AdminAvailabilityStrip dishes={data.menu.dishes}/></AdminPanel></div>
  </AdminShell>;
}
