...
export function AdminTopDishes({ evidence, dishes }: { evidence: AdminPanelEvidence<Ranked[]>; dishes: Map<string, Dish> }) {
  ...
  return <ol className={styles.ranking} data-overview-ranking>{evidence.data.slice(0, 5).map((item, index) => {
    ...
    return <li key={item.slug}>
      ...
      <div><strong title={name}>{name}</strong><span>{item.count} consultation{item.count > 1 ? "s" : ""}</span><i data-chart-animated="rank-bar" style={{ "--value": `${item.count / max * 100}%` } as React.CSSProperties}/></div>
    </li>;
  })}</ol>;
}
