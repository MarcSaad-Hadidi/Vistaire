...
export function AdminAvailabilityStrip({ dishes }: { dishes: AdminMenuDish[] }) {
  return <div className={styles.dishStrip}>
    {dishes.slice(0, 5).map((dish) => <article key={dish.id} data-overview-availability-card>
      <AdminDishThumbnail compact name={dish.name} thumbnailUrl={dish.thumbnailUrl} imageUrl={dish.imageUrl} sizes="64px"/>
      <div><strong>{dish.name}</strong><span>{dish.priceLabel}</span><AdminStatusBadge tone={dish.available ? "available" : "unavailable"}>{dish.available ? "Disponible" : "Indisponible"}</AdminStatusBadge></div>
