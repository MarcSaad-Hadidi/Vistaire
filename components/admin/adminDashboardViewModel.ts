...
export function buildAnalyticsPresentation(state: TargetAnalyticsState) {
  switch (state.kind) {
    case "real": {
      const total = state.metrics.reduce((sum, metric) => sum + metric.value, 0);
      return { kind: "real" as const, completeness: state.completeness, metrics: state.metrics, activity: state.activitySeries, summary: `${total} signaux mesurés sur la période.`, provenance: state.coverage.provenance ?? state.coverage.source ?? "production", lastUpdatedAt: state.lastUpdatedAt, freshness: state.freshness };
    }
    case "insufficient":
