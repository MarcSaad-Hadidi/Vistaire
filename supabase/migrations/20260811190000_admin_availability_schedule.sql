...
create table if not exists public.admin_dish_availability_schedules (
  ...
  internal_note text check (internal_note is null or length(internal_note) <= 120),
...
alter table public.admin_dish_availability_events
  drop constraint if exists admin_availability_events_schedule_fk;
alter table public.admin_dish_availability_events
  add constraint admin_availability_events_schedule_fk foreign key (schedule_id)
  references public.admin_dish_availability_schedules(id) not valid;
alter table public.admin_dish_availability_events
  validate constraint admin_availability_events_schedule_fk;
...
$$;
drop trigger if exists admin_availability_events_append_only on public.admin_dish_availability_events;
...
create or replace function public.schedule_admin_dish_availability(
  ...
  p_available boolean, p_scheduled_for timestamptz, p_timezone text, p_idempotency_key text, p_internal_note text default null
)
...
begin
  ...
  if p_internal_note is not null and length(p_internal_note) > 120 then raise exception 'note too long'; end if;
  insert into public.admin_dish_availability_schedules(restaurant_id,menu_id,dish_id,final_available,scheduled_for,timezone,idempotency_key,requester_qr_id,internal_note)
  values(p_restaurant_id,p_menu_id,p_dish_id,p_available,p_scheduled_for,p_timezone,p_idempotency_key,p_qr_id,nullif(trim(p_internal_note),''))
...
create index if not exists admin_availability_schedules_due_idx
  on public.admin_dish_availability_schedules (scheduled_for, id)
  where status = 'pending';
create index if not exists admin_availability_events_scope_idx
  on public.admin_dish_availability_events (restaurant_id, menu_id, created_at desc);

create or replace function public.cancel_admin_dish_availability(p_qr_id uuid, p_restaurant_id uuid, p_dish_id uuid, p_schedule_id uuid)
...
begin
  ...
  update public.admin_dish_availability_schedules s set status='cancelled' where s.id=p_schedule_id and s.restaurant_id=p_restaurant_id and s.dish_id=p_dish_id and s.status='pending';
  return found;
end;
$$;

create or replace function public.mark_admin_availability_worker_attempt(p_worker_id text)
returns void language plpgsql security definer set search_path = '' as $$
begin
  if p_worker_id <> 'primary' then raise exception 'invalid worker'; end if;
  insert into public.admin_availability_workers(worker_id,schema_version,last_attempt_at)
  values(p_worker_id,1,now())
  on conflict(worker_id) do update set schema_version=excluded.schema_version,last_attempt_at=excluded.last_attempt_at;
end;
$$;
...
revoke execute on function public.schedule_admin_dish_availability(uuid,uuid,uuid,uuid,boolean,timestamptz,text,text,text) from public, anon, authenticated;
revoke execute on function public.cancel_admin_dish_availability(uuid,uuid,uuid,uuid) from public, anon, authenticated;
revoke execute on function public.mark_admin_availability_worker_attempt(text) from public, anon, authenticated;
...
grant execute on function public.schedule_admin_dish_availability(uuid,uuid,uuid,uuid,boolean,timestamptz,text,text,text) to service_role;
grant execute on function public.cancel_admin_dish_availability(uuid,uuid,uuid,uuid) to service_role;
grant execute on function public.mark_admin_availability_worker_attempt(text) to service_role;
